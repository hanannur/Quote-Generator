...
/*
  Make sure to import fetchNewQuote from its source file in the parent component,
  and pass it as a prop to ActionButtons. For example:
  import { fetchNewQuote } from '../path/to/fetchNewQuote';
  <ActionButtons fetchNewQuote={fetchNewQuote} ... />
*/
...
export const ActionButtons = ({ isLoading, copyText, fetchNewQuote, copyToClipboard }) => (
    <div className="flex flex-col sm:flex-row justify-center sm:justify-end gap-4 mt-8 pt-4 border-t border-gray-100">
        ...
        <button 
            onClick={() => {
                if (!isLoading && typeof fetchNewQuote === 'function') {
                    fetchNewQuote();
                }
            }}
            disabled={isLoading}
